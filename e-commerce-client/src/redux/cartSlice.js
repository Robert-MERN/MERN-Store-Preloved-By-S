...
const cartSlice = createSlice({
    ...
    reducers: {
           ...
           if (state.products){
               ...
               if(customSameProduct[0]){
                    ...
                    if(action.payload.operate){
                        let remained = similarProduct[0].sellingPrice * similarProduct[0].Quantity;
                        let writeState = state.total;
                        state.total -= remained;
                        if(writeState > state.total){
                            state.total += action.payload.sellingPrice * action.payload.Quantity;
                        }
                    }
                } else {
        ...
        addTotal: (state, action) => {
                state.total += action.payload.sellingPrice;
        },
